...
def test_m18_models_versioned_regulatory_transition_and_remediation():
 ...
 with psycopg.connect(**p.connection_kwargs()) as c:
  ...
  assert r=={'regimes':3,'affected_count':200,'remediation_count':100}
  assert c.execute("SELECT count(*) FROM resilience_v2.regulatory_regimes WHERE tenant_id=%s",(tenant,)).fetchone()[0]==3
